...
#print(os.getcwd())
data_path = os.path.join( "./data", "project_2", "eod-quotemedia.csv")
#!ls $data_path

df_original = pd.read_csv(data_path, parse_dates=['date'], index_col=False)
...
def filter_signals(signal, lookahead_days):
    ...
    signalcopy = pd.DataFrame(data=0,columns=signal.columns,index=signal.index)
    for col in signal.columns:
        serBuys = (signal[col]+1)//2
        sigBuys = clear_signals(serBuys,lookahead_days)

        serSells = np.ceil((signal[col]-1)/2).astype(np.int64)
        sigSells = clear_signals(serSells,lookahead_days)

        signalcopy[col] = sigSells+sigBuys
    return signalcopy.astype(np.int64)
...
# 
# 
# From the histogram, the signal return values is not in normal distribution but right-skewed distribution which is because of the effect of outliers.
...
def calculate_kstest(long_short_signal_returns):
    ...
    ks_v = []
    p_v = []
    for col, group in long_short_signal_returns.groupby('ticker'):   
        sample = group['signal_return'].values
        mean = long_short_signal_returns['signal_return'].mean()
        # set the delta degrees of freedom to 0
        std = long_short_signal_returns['signal_return'].std(ddof=0)
        ks_value, p_value = kstest(sample,"norm", (mean, std))
        
        column.append(col)
        ks_v.append(ks_value)
        p_v.append(p_value)
        
    return pd.Series(ks_v, column), pd.Series(p_v, column)
...
def find_outliers(ks_values, p_values, ks_threshold, pvalue_threshold=0.05):
    ...
    outl1 = ks_values.where(ks_values > ks_threshold).dropna()
    outl2 = p_values.where(p_values < pvalue_threshold).dropna()
    #intersection of two sets is our answer
    outliers = set(outl1.index) & set(outl2.index)
    return outliers
