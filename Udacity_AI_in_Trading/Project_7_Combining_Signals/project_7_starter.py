...
#import os
#barra_dir = os.path.join(os.getcwd(), '..', '..', 'data', 'project_7_eod') 
#!ls $barra_dir
#!tar -zcvf data.tar.gz $barra_dir 
#!ls
#!cp ../../data/project_8_barra/pandas-frames.2003.pickle .
...
# 
# In the graph, we can see higher autocorrelation values with higher shifting days e.g. ~0.75 for target_1(Shifting 4 days). Also, there is a high corralation between these shifted labels.
...
def train_valid_test_split(all_x, all_y, train_size, valid_size, test_size):
    ...
    # TODO: Implement
    ########################
    time_index = all_x.index.levels[0]
    
    train_index = time_index[:int(len(time_index)*train_size)]
    valid_test_index = np.setdiff1d(time_index, train_index)

    valid_index = valid_test_index[ : int(len(valid_test_index)*(valid_size/(valid_size+test_size)))]
    test_index = np.setdiff1d(valid_test_index, valid_index)
    
    x_train = all_x[np.in1d(all_x.index.get_level_values(0), train_index)]
    x_valid = all_x[np.in1d(all_x.index.get_level_values(0), valid_index)]
    x_test = all_x[np.in1d(all_x.index.get_level_values(0), test_index)]
    
    y_train = all_y[np.in1d(all_y.index.get_level_values(0), train_index)]
    y_valid = all_y[np.in1d(all_y.index.get_level_values(0), valid_index)]
    y_test = all_y[np.in1d(all_y.index.get_level_values(0), test_index)]
...
# *#TODO: Put Answer In this Cell*
# 
# The feature in the first split does not mean to have the highest feature importance. In this table above, we can see that the feature "dispersion_20d" has the higher feature importance as well as information gain as it is used in multiple nodes as criteria for the splitting while "Momentum_1YR" feature has the largest information gain for one split only.
...
# *#TODO: Put Answer In this Cell*
# 
# When the number of trees increase, the acc. in training increases while the acc. in validation decreases. The model seem to overfit as the validation accuracy seems to decline.
# 
# 
...
def non_overlapping_samples(x, y, n_skip_samples, start_i=0):
    ...
    drop_index = []
    target_index = [start_i] 
    target = start_i 
    for i in range(len(x.index.levels[0])):
        drop_index.append(i)
        target += n_skip_samples + 1
        if target <= len(x.index.levels[0]):
            target_index.append(target)
    
    for i in target_index:
        if i in drop_index:
            drop_index.remove(i)
    non_overlapping_x = x.drop(x.index.levels[0][drop_index])
    non_overlapping_y = y.drop(y.index.levels[0][drop_index])
...
def bagging_classifier(n_estimators, max_samples, max_features, parameters):
    ...
    # TODO: Implement
    ####################
    clf = DecisionTreeClassifier(criterion = parameters["criterion"], min_samples_leaf = parameters["min_samples_leaf"])
                             
    clf = BaggingClassifier(base_estimator=clf, n_estimators=n_estimators,
                            max_features=max_features, max_samples=max_samples, oob_score = parameters["oob_score"],
                            n_jobs = parameters["n_jobs"], random_state = parameters["random_state"])
    return clf
...
def calculate_oob_score(classifiers):
    ...
    for model in classifiers:
        oob_score_list.append(model.oob_score_)
    
    oob_score = np.mean(oob_score_list)
    return oob_score
...
def non_overlapping_estimators(x, y, classifiers, n_skip_samples):
    ...
    fit_classifiers = [classifiers[i].fit(*non_overlapping_samples(x, y, n_skip_samples, start_i=i)) for i in range(len(classifiers))]
