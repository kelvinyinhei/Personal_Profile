...
def calculate_dividend_weights(dividends):
    ...
    # Create a copy
    cum_dividends = dividends.copy()

    for i in range(len(dividends)):
        row = dividends.iloc[i, :]
        row_sum = row.sum()
        cum_dividends.iloc[i, :] = row_sum
    
    dividend_weights = dividends.cumsum() / cum_dividends.cumsum() 
...
def calculate_cumulative_returns(returns):
    ...
    cumulative_returns = returns.sum(axis=1) + 1
    for i, (index, val) in enumerate(cumulative_returns.iteritems()):
        if i - 1 < 0 or pd.isnull(cumulative_returns[i-1]):
            continue
        cumulative_returns[i] =  cumulative_returns[i] * cumulative_returns[i-1]
...
def tracking_error(benchmark_returns_by_date, etf_returns_by_date):
    ...
    # Rmb to set "delta degrees of freedom is 1"
    tracking_error = 252**(1/2) * np.std(etf_returns_by_date - benchmark_returns_by_date, ddof=1)
...
def get_covariance_returns(returns):
    ...
    # Cal 
    returns_covariance = returns_covariance.cov().values
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    for i, (index, row) in enumerate(returns.iterrows()):
        shift_count += 1
        if i < chunk_size:
            shift_count = shift_size - 1
            continue
        if shift_count == shift_size:
            start = i - chunk_size
            #print(returns[start:i])
            #print(index)
            #print(index_weights.iloc[i-1])
            x = get_optimal_weights(get_covariance_returns(returns[start:i]), index_weights.iloc[i-1])
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    #print(shift_size)
    #print(all_rebalance_weights)
    #print(np.roll(all_rebalance_weights, shift_size, axis=1))
    #sum_total_turnover = np.sum(all_rebalance_weights - np.roll(all_rebalance_weights, shift_size, axis=0))
    #print(sum_total_turnover)
    #annualized_turnover = (sum_total_turnover / rebalance_count) * (rebalance_count/n_trading_days_in_year )
